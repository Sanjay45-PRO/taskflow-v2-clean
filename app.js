...
let liveMapRenderGeneration = 0;

async function loadLiveMap(){
  const myGeneration = ++liveMapRenderGeneration;
  ...
  // If a newer call has started since this one began, stop here — don't let
  // this stale request draw anything on top of the newer, correct result.
  if (myGeneration !== liveMapRenderGeneration) return;
  ...
  for (let i = 0; i < names.length; i++){
    ...
    // A newer click/refresh may have started while this was fetching — if so,
    // this result is stale and must never be drawn on top of the current one.
    if (myGeneration !== liveMapRenderGeneration) return;
